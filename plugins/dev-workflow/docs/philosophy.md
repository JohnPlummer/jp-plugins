...
> This is the plugin's opinion: always on, not overridable by standards (unlike the standards cascade, which is external and repo-overridable).
...
1. **Separation of judgment** *(the crown jewel)* - a single agent that writes the code, the tests, and judges completion Goodharts its own artefacts. Isolate the roles: test-author != implementer != completion-judge; the judge evaluates against the **spec**, not the tests. Maps directly onto Workflow subagent isolation.
...
These principles come from documented engineering practice, not invention: AI-native DDD/BDD/TDD, separation of judgment, and trunk-based working agreements. This doc is their canonical home.
