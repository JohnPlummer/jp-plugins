...
1. **Machine default** - `$LINEAR_*` exported from `~/.zshenv.local`, the dotfiles' machine-local file (sourced by `.zshenv`, never committed). A machine belongs to one Linear workspace, and its MCP is authenticated against that workspace, so this is the everyday default: a personal Mac resolves to the personal workspace and team, a work Mac to the work one. Most repos need nothing else. It cannot live under `~/.claude`, which is shared across machines through git - the workspaces differ, and so do the home directories. Use `.zshenv.local` rather than `.zshrc.local`: `.zshrc` is skipped by non-interactive shells, so a value set there is invisible to any tool that shells out without a tty. The script still reads `~/.zshrc.local` as a legacy fallback for machines that have not migrated.
...
The script reads the repo's settings file directly for layer 2 rather than trusting `$LINEAR_*` in the environment, so a repo bound to another workspace always beats the machine default regardless of how Claude Code orders its env injection - the inversion that would cause a wrong-workspace write. If neither layer resolves, the script fails and the skill stops rather than guessing a team.
